...
window.onload = function() {
    ...
    const allCustomizationInputs = [fogToggle, mountainPercent, forestPercent, cityCount];
    ...
    function showModal(config) {
        ...
        if (config.showInput) {
            ...
            setTimeout(() => modalInput.focus(), 10); // Auto-focus input
        }
    ...
    socket.on('game-state-update', (newState) => {
        ...
        updatePlayerListView(null); // No host during game
    });
    ...
    function updatePlayerListView(hostPlayerId) {
        ...
        if (playerListElement) {
            ...
            players.forEach(player => {
                ...
                // Show army count only if game is running
                const armyDisplay = localGameState.multiverse[activeTimelineId] ? `(Army: ${armyCount})` : '';
    ...
    // ... getCausalityColor, render, renderTimelineTree, and input handlers remain largely unchanged ...
    ...
    // ... keydown and other button listeners ...
